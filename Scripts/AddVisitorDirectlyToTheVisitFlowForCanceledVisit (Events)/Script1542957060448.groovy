...
if (!(WebUI.verifyElementPresent(findTestObject('DailyVisitsPage/CancelDailyVisitButton'), 0, FailureHandling.OPTIONAL))) {
	KeywordUtil.markFailed('Cancel visit button is not present in details page of Event')
...
//    KeywordUtil.markFailed('Cancel Event modal is not shown when user clicks Cancel Daily visit button')
//
//    WebUI.takeScreenshot()
//}
...
if (!(WebUI.verifyElementNotPresent(findTestObject('DailyVisitsPage/CancelNowButtonInCancelVisitModal'), 0, FailureHandling.OPTIONAL))) {
	KeywordUtil.markFailed('After selecting the check box and clicking Cancel Now button in Cancel Event modal, the modal is not getting closed')
...
not_run: CustomKeywords.'com.CommonUtilities.FilterInIndexPage.ApplyFiltersInIndexPage'(VisitType)
