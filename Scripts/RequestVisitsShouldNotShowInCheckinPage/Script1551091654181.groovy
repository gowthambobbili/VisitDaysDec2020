...
CustomKeywords.'com.CommonUtilities.Verifications.VerifyElementPresentAndStopExecutionIfFailed'(findTestObject('HomePageElements/CheckInLeftNav'),
		'Check In button is not present in the home page')
...
CustomKeywords.'com.CommonUtilities.Verifications.VerifyElementPresentAndStopExecutionIfFailed'(findTestObject('CheckInPage/TodaysDateInCheckInPage'),
		'When clicked Check In button the date is not shown in the check in page')

def RequesVisitTypeInCHeckInPage = new TestObject('Request visit type')

RequesVisitTypeInCHeckInPage.addProperty('xpath', ConditionType.EQUALS, ('//h4[text()=\'' + RequestVisitType) + '\']')

not_run: CustomKeywords.'com.CommonUtilities.Verifications.VerifyElementNotPresentStopExecutionIfFailed'(RequesVisitType,
'Request visits are shown in Check In page')

if(WebUI.verifyElementPresent(RequesVisitTypeInCHeckInPage, 10, FailureHandling.OPTIONAL))
{
	WebUI.takeScreenshot()
	KeywordUtil.markFailedAndStop("Request visits are shown in Check In page")
}
