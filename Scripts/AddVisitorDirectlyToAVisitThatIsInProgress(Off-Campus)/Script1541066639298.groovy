...
CustomKeywords.'com.CommonUtilities.CreateVisitWithDifferentStatuses.CreateOffCampusWithInprogressTimes'(VisitType)
...
if (!(WebUI.verifyElementPresent(status, 10, FailureHandling.OPTIONAL))) {
	KeywordUtil.markFailed('After adding an individual visitor to an Event which is in progress then the user is not checked in')
